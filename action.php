...
if(isset($_POST["category"])){
	...
	echo "
            <div class='aside'><h3 class='aside-title'>Categories</h3><div class='btn-group-vertical'>";
	...
	if(mysqli_num_rows($run_query) > 0){
		...
		while($row = mysqli_fetch_array($run_query)){ 
            ...
            $i++;
			echo "	
                    <div type='button' class='btn navbar-btn category' cid='$cid'><a href='#'><span  ></span>
					$cat_name<small class='qty'>($count)</small></a></div>";
				}
		echo "</div>";
	}
}
...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='aside'><h3 class='aside-title'>Brand</h3><div class='btn-group-vertical'>
	";
	if(mysqli_num_rows($run_query) > 0){
        $i=1;
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "<div type='button' class='btn navbar-btn selectBrand' bid='$bid'>
					<a href='#'><span ></span>$brand_name<small >($count)</small></a></div>";
		    }
		echo "</div>";
	}
}
...
if(isset($_POST["getProduct"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
            ...
            $cat_name = $row["cat_title"];
			echo "<div class='col-md-4 col-xs-6' ><a href='product.php?p=$pro_id'><div class='product'>
